 ...
 * @version $Revision$
 */
public class NDLTDDescribeStep extends AbstractProcessingStep
{
    /** log4j logger */
    ...
    /** Constructor */
    public NDLTDDescribeStep() throws ServletException
    {
            ...
            AuthorizeException
    {
                ...
                || buttonPressed.equals(CANCEL_BUTTON))
        {
            for (int i = 0; i < inputs.length; i++)
            {
                ...
                // new added, get metadata value of "dc.contributor.department", automatically add "thesis.degree.discipline"
                if (inputs[i].getSchema().equals("dc") 
                		&& inputs[i].getElement().equals("contributor") 
                		&& qualifier.equals("department")) {
                	
                    item.clearMetadata("thesis", "degree", "discipline", Item.ANY);
                    item.addMetadata("thesis", "degree", "discipline", LANGUAGE_QUALIFIER, values[0].value);
                }
            }
        }
